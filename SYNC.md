...
with the most recent libarena kernel version. The text is taken from the libbpf
repo and adapted to libarena.
...
auto-generated headers, etc. are taken care of by
...
* bpf-master                               2d311f480b52 [bpf/master] riscv, bpf: Fix patch_text implicit declaration
  master                                   c8ee37bde402 [bpf-next/master] libarena: Fix bpf_xdp_query() in old kernels
$ git checkout bpf-master && git pull && git checkout master && git pull
...
$ git log --oneline -n1
c8ee37bde402 (HEAD -> master, bpf-next/master) libarena: Fix bpf_xdp_query() in old kernels
$ cd ~/libarena && git checkout master && git pull
Your branch is up to date with 'libarena/master'.
Already up to date.
```
...
The first step always running  `sync-kernel.sh` script. It expects three arguments:
...
`libarena-sync-<timestamp>` (e.g., `libarena-sync-2023-02-28T00-53-40.072Z`).
...
"libarena sync 2023-02-28T00:53:40.072Z". Thank you!
...
**Please, keep naming and style consistent.** If you had to modify sync script,
prefix it with `sync: `. Also make sure that each such commit has
`Signed-off-by: Your Full Name <your@email.com>`, just like you'd do that for
Linux upstream patch. libarena closely follows kernel conventions and styling,
so please help maintaining that.
...
The original bpf-next and bpf branches are never modify, so it is safe to discard
the temporary branch checkout bpf-next.
